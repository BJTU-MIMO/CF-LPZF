function [DS, BU, PC, MU, GN, QN, A] = functionComputeSE_AP_uplink_simu_ADC_LPZF(Hhat,H,R,C,nbrOfRealizations,N,K,L,D,p_uplink,alpha,pilotIndex,H_bar,tau_p,S,W,Z,M,E_S_temp,c_matrix)
...
%%
G_ki  = zeros(L,K,K,4,nbrOfRealizations)  ;
G_kiki= zeros(L,L,K,K,4,nbrOfRealizations);

f2_k  = zeros(L,K,4,nbrOfRealizations)    ;
f2_kk = zeros(L,L,K,4,nbrOfRealizations)  ;

f1_k  = zeros(L,K,4,nbrOfRealizations)    ;
f1_kk = zeros(L,L,K,4,nbrOfRealizations)  ;

E_numer       =  zeros(L,K,4)      ;
E_denumer_gki =  zeros(L,L,K,K,4)  ;
E_denumer_f2k =  zeros(L,L,K,4)    ;
E_denumer_f1k =  zeros(L,L,K,4)    ;
...
for k = 1:K
    ...
    for n = 1:nbrOfRealizations
        ...
        for l_Z_k = 1:Z_k_ind_length
            ...
            V_LPZF(:,l_Z_k,k)  = squeeze(H_bar((Z_k_ind(l_Z_k)-1)*N+1:Z_k_ind(l_Z_k)*N,n,:))*E_S_l/(E_S_l'*squeeze(H_bar((Z_k_ind(l_Z_k)-1)*N+1:Z_k_ind(l_Z_k)*N,n,:))'*squeeze(H_bar((Z_k_ind(l_Z_k)-1)*N+1:Z_k_ind(l_Z_k)*N,n,:))*E_S_l)*epso;
