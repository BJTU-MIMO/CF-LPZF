...
SE_AP_LPPWZF_tot = zeros(K,2,nbrOfSetups);
SE_AP_LPPSZF_tot = zeros(K,2,nbrOfSetups);
...
for n = 1:nbrOfSetups
   ...
   [gainOverNoisedB,R,pilotIndex,D_orig] = generateSetup_new(L,K,N,tau_p,1);
    ...
    %p_uplink = fractionalPowerControl(beta_matrix,p_max,K);
    p_uplink = p_max*ones(K,1);
    p_regu = p_uplink;
    ...
    [S,W,Z,M_matrix,E_S_temp,E_W_temp,] = functionUEgrouping_1123(L,K,beta_matrix,pilotIndex,tau_p);
   ...
   [SE_LPZF,SE_LPPWZF,SE_LPPSZF] = functionComputeSE_AP_uplink_lowADC_LPZF_LPPZF_final_PWandPS1123(Hhat,H,R,tau_c,tau_p,nbrOfRealizations,N,K,L,p_uplink,alpha,pilotIndex,H_bar,S,W,Z,M_matrix,E_S_temp,E_W_temp,c_matrix);
    ...
    SE_AP_LPPWZF_tot(:,:,n) = SE_LPPWZF;
    SE_AP_LPPSZF_tot(:,:,n) = SE_LPPSZF;
...
plot(sort(reshape(SE_AP_LPPWZF_tot(:,1,:),[K*nbrOfSetups 1])),linspace(0,1,K*nbrOfSetups),'g-','LineWidth',5);
plot(sort(reshape(SE_AP_LPPSZF_tot(:,1,:),[K*nbrOfSetups 1])),linspace(0,1,K*nbrOfSetups),'y-','LineWidth',5);
xlabel('Spectral efficiency [bit/s/Hz]','Interpreter','Latex');
ylabel('CDF','Interpreter','Latex');
legend({'L3 (MR)','L3 (L-MMSE) ','L3 (FZF) ','L3 (RZF) ','L3 (LPZF) ','L3 (LPPWZF) ','L3 (LPPSZF) '},'Interpreter','Latex','Location','NorthWest');
