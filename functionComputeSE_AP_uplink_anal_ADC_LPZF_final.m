function [DS, BU, PC, MU, GN, QN, A_finalre] = functionComputeSE_AP_uplink_anal_ADC_LPZF_final(Psi,R,K,L,D,p_uplink,alpha,pilotIndex, p_pilot,tau_p,N,c_matrix,gamma_matrix, beta_matrix, theta_matrix, S,W,Z,M,E_S_temp)
...
%     temp_2(find(temp_2==0))=[];
    ...
    M_k = M(k,:);
    for l = 1: L
        A_BU_LPZF = computeBU_LPZF(E_S_temp, alpha, p_uplink, A_A, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, Z_k, M_k, k, L);
        [~, A_PC_LPZF] = computePC_LPZF(E_S_temp, alpha, p_uplink, c_matrix, Psi, A_A, Z_k, M_k, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, L, pilotIndex, K, k);
        A_MU_LPZF = computeMU_LPZF(E_S_temp, alpha, p_uplink, A_A, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, Z_k, M_k, pilotIndex, K, L, k);
        A_GN_LPZF =  computeGN_LPZF(E_S_temp, alpha, A_A, N, tau_p, theta_matrix, gamma_matrix, Z_k, M_k, L, k);
        A_QN_LPZF = computeQN_LPZF(E_S_temp, alpha, A_A, p_uplink, beta_matrix, N, tau_p, theta_matrix, Z_k, M_k, gamma_matrix, k, L, K);
    ...
    for k1 = 1:K
        if pilotIndex(k1)==pilotIndex(k)
            if k1~=k
                b_k_vec = computeA_part2_LPZF(E_S_temp, c_matrix, Psi, Z_k, M_k, k1, L,pilotIndex, k);
                A_part2_LPZF(:,:,k) = A_part2_LPZF(:,:,k) + alpha^2*p_uplink(k1)*(b_k_vec*b_k_vec');
            end
        end
    end
    A_part1_LPZF(:,:,k) = A_part1_LPZF(:,:,k) + A_part2_LPZF(:,:,k);
    A_part3_LPZF(:,k) = computeA_part2_LPZF(E_S_temp, c_matrix, Psi, Z_k, M_k, k, L,pilotIndex, k);
    ...
    DS(k,:,3) = computeDS_LPZF(E_S_temp, alpha, A_LPZF, Z_k, c_matrix, M_k, N, gamma_matrix, p_uplink, k);
    BU(k,:,3) = sum(computeBU_LPZF(E_S_temp, alpha, p_uplink, A_LPZF, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, Z_k, M_k, k, L))';
    [PC_all, ~] = computePC_LPZF(E_S_temp, alpha, p_uplink, c_matrix, Psi, A_LPZF, Z_k, M_k, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, L, pilotIndex, K, k);
    PC(k,:,3) = PC_all;    
    MU(k,:,3) = sum(computeMU_LPZF(E_S_temp, alpha, p_uplink, A_LPZF, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, Z_k, M_k, pilotIndex, K, L, k))';
    GN(k,:,3) = sum(computeGN_LPZF(E_S_temp, alpha, A_LPZF, N, tau_p, theta_matrix, gamma_matrix, Z_k, M_k, L, k))';
    QN(k,:,3) = sum(computeQN_LPZF(E_S_temp, alpha, A_LPZF, p_uplink, beta_matrix, N, tau_p, theta_matrix, Z_k, M_k, gamma_matrix, k, L, K))';
...
function DS_LPZF = computeDS_LPZF(E_S_temp, alpha, A_LPZF, Z_k, c_matrix, M_k, N, gamma_matrix, p_uplink, k)
...
function BU_LPZF = computeBU_LPZF(E_S_temp, alpha, p_uplink, A_LPZF, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, Z_k, M_k, k, L)
...
for l_Z_k = 1:length(Z_k_ind)
    ...
    BU_LPZF(l_Z_k,1) = alpha^2*p_uplink(k)*abs(A_LPZF(l_Z_k,k,1))^2*(beta_matrix(Z_k_ind(l_Z_k),k)-gamma_matrix(Z_k_ind(l_Z_k),k))/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));
    BU_LPZF(l_Z_k,2) = alpha^2*p_uplink(k)*abs(A_LPZF(l_Z_k,k,2))^2*(beta_matrix(Z_k_ind(l_Z_k),k)-gamma_matrix(Z_k_ind(l_Z_k),k))/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));  
end
for l_M_k = 1:length(M_k_ind)
...
function [PC_LPZF_all,PC_LPZF_part] = computePC_LPZF(E_S_temp, alpha, p_uplink, c_matrix, Psi, A_LPZF, Z_k, M_k, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, L, pilotIndex, K, k)
    ...
    if pilotIndex(i) == pilotIndex(k)
        if i ~= k
            for l_Z_k = 1:length(Z_k_ind)
                ...
                PC_LPZF_part(l_Z_k,1) = PC_LPZF_part(l_Z_k,1) + alpha^2*p_uplink(i)*abs(A_LPZF(l_Z_k,k,1))^2*(beta_matrix(Z_k_ind(l_Z_k),i)-gamma_matrix(Z_k_ind(l_Z_k),i))/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));
                PC_LPZF_part(l_Z_k,2) = PC_LPZF_part(l_Z_k,2) + alpha^2*p_uplink(i)*abs(A_LPZF(l_Z_k,k,2))^2*(beta_matrix(Z_k_ind(l_Z_k),i)-gamma_matrix(Z_k_ind(l_Z_k),i))/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));
            end
...
function  MU_LPZF = computeMU_LPZF(E_S_temp, alpha, p_uplink, A_LPZF, beta_matrix, gamma_matrix, N, tau_p, theta_matrix, Z_k, M_k, pilotIndex, K, L, k)
...
for i = 1:K
    if pilotIndex(i) ~= pilotIndex(k)
        for l_Z_k = 1:length(Z_k_ind)
            E_S_l_ind = find(E_S_temp(Z_k_ind(l_Z_k),:)~=0);
            tau_S_l = length(E_S_l_ind);
            MU_LPZF(l_Z_k,1) = MU_LPZF(l_Z_k,1) + alpha^2*p_uplink(i)*abs(A_LPZF(l_Z_k,k,1))^2*(beta_matrix(Z_k_ind(l_Z_k),i)-gamma_matrix(Z_k_ind(l_Z_k),i))/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));
            MU_LPZF(l_Z_k,2) = MU_LPZF(l_Z_k,2) + alpha^2*p_uplink(i)*abs(A_LPZF(l_Z_k,k,2))^2*(beta_matrix(Z_k_ind(l_Z_k),i)-gamma_matrix(Z_k_ind(l_Z_k),i))/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));
        end
        for l_M_k = 1:length(M_k_ind)
...
function GN_LPZF = computeGN_LPZF(E_S_temp, alpha, A_LPZF, N, tau_p, theta_matrix, gamma_matrix, Z_k, M_k, L, k)
...
for l_Z_k = 1:length(Z_k_ind)
    E_S_l_ind = find(E_S_temp(Z_k_ind(l_Z_k),:)~=0);
    tau_S_l = length(E_S_l_ind);
    GN_LPZF(l_Z_k,1) = GN_LPZF(l_Z_k,1) + alpha^2*abs(A_LPZF(l_Z_k,k,1))^2/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));
    GN_LPZF(l_Z_k,2) = GN_LPZF(l_Z_k,2) + alpha^2*abs(A_LPZF(l_Z_k,k,2))^2/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));    
end
for l_M_k = 1:length(M_k_ind)
...
function QN_LPZF = computeQN_LPZF(E_S_temp, alpha, A_LPZF, p_uplink, beta_matrix, N, tau_p, theta_matrix, Z_k, M_k, gamma_matrix, k, L, K)
...
for l_Z_k = 1:length(Z_k_ind)
    ...
    temp_1 = temp_1 + 1;
    QN_LPZF(l_Z_k,1) = QN_LPZF(l_Z_k,1) + alpha*(1-alpha)*abs(A_LPZF(l_Z_k,k,1))^2*temp_1/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));
    QN_LPZF(l_Z_k,2) = QN_LPZF(l_Z_k,2) + alpha*(1-alpha)*abs(A_LPZF(l_Z_k,k,2))^2*temp_1/((N-tau_S_l)*theta_matrix(Z_k_ind(l_Z_k),k));   
end
for l_M_k = 1:length(M_k_ind)
    temp_2 = 0; 
    for i = 1:K
...
function b_k = computeA_part2_LPZF(E_S_temp, c_matrix, Psi, Z_k, M_k, k1, L,pilotIndex, k)
