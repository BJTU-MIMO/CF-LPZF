function [SE_mRZF_simu, SE_mRZF_anal] = functionComputeSE_AP_uplink_mRZF(Hhat,H,H_bar,tau_c,tau_p,nbrOfRealizations,N,K,L,p_uplink,pilotIndex,beta_matrix,gamma_matrix,theta_matrix,c_matrix)
    ...
    for i = 1:K
        ...
        if pilotIndex(i) ~= pilotIndex(k)
            ...
            for l  = 1:L
                ...
                UI(k) = UI(k) + (1/N)*p_uplink(i)*a^2*c_matrix(l,k)^2*(c_matrix(l,i)^2*Big_e_hat_matrix(pilotIndex(i),pilotIndex(k),l) + a^2*(beta_matrix(l,i)-gamma_matrix(l,i))*e_hat_matrix(pilotIndex(i),l)*(1+e_matrix(pilotIndex(i),l))^2)/(1+e_matrix(pilotIndex(k),l))^2/(1+e_matrix(pilotIndex(i),l))^2;
